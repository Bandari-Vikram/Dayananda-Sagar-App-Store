...
import { DUMMY_EMAIL, DUMMY_PASSWORD } from "@/lib/auth";
...
function LoginPageContent() {
  ...
  const nextPath = searchParams.get("next") || "/home/explore";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(result.error ?? "Login failed. Try demo@dsu.com / demo");
    setIsLoading(false);
  };
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex items-center justify-center p-8 bg-background")}>
        <div className="w-full max-w-[420px]">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground text-sm">Dummy login for testing — use demo credentials below</p>
          </div>

          <div className="mb-5 rounded-xl border border-indigo-500/30 bg-indigo-500/10 px-4 py-3 text-left text-sm text-foreground/85">
            <p className="font-semibold text-foreground">Demo login</p>
            <p className="mt-1">
              Email: <span className="font-mono">{DUMMY_EMAIL}</span>
            </p>
            <p>
              Password: <span className="font-mono">{DUMMY_PASSWORD}</span>
            </p>
            <p className="mt-2 text-xs text-foreground/60">
              Or any email with @ and any password (3+ letters).
            </p>
          </div>
          ...
          <div className="mt-4">
            <Button
              type="button"
              variant="outline"
              className="w-full h-12 border-indigo-500/40 bg-indigo-500/5 hover:bg-indigo-500/15"
              disabled={isLoading}
              onClick={handleDemoLogin}
            >
              {isLoading ? "Please wait…" : "Quick Demo Login (no form)"}
            </Button>
          </div>
